...
uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
  ...
  for(a = va; a < va + npages*PGSIZE; a += sz){
    ...
    if((*pte & PTE_V) == 0) {
      printf("uvmunmap: not mapped va=0x%lx pte=0x%lx\n", a, *pte);
    ...
    #ifdef LAB_PGTBL
    // 检查是否是超级页（Level 1 的叶节点）
    // walk() 返回的 pte 可能是 Level 1 的
    pagetable_t l2 = pagetable;
    pte_t *l2_pte = &l2[PX(2, a)];
    if(*l2_pte & PTE_V) {
      pagetable_t l1 = (pagetable_t)PTE2PA(*l2_pte);
      pte_t *l1_pte = &l1[PX(1, a)];
      
      // 如果 Level 1 有叶节点 → 超级页
      if((*l1_pte & PTE_V) && PTE_LEAF(*l1_pte)) {
        if(do_free) {
          uint64 pa = PTE2PA(*l1_pte);
          superfree((void*)pa);  // 释放 2MB
        }
        *l1_pte = 0;
        sz = SUPERPGSIZE;  // 跳过 2MB
        continue;
      }
    }
    #endif
  ...
  for(a = oldsz; a < newsz; a += sz){
    ...
    if((a % SUPERPGSIZE == 0) && (newsz - a >= SUPERPGSIZE)) {
      ...
      if(mem != 0) {
        ...
        #ifndef LAB_SYSCALL
        memset(mem, 0, sz);  // 清零 2MB
        #endif
        
        // 映射超级页（size=SUPERPGSIZE 会触发超级页分支）
        ...
        continue;  // 跳过普通页分配，继续下一轮
      } else {
        printf("uvmalloc: superalloc FAILED, fallback to 4KB\n");
      }
      // 如果 superalloc() 失败，回退到普通页分配
    }
    #endif
...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += szinc){
    ...
    // ---- 检查是否是超级页 ----
    pagetable_t l2 = old;
    pte_t *l2_pte = &l2[PX(2, i)];
    
    if(*l2_pte & PTE_V) {
      pagetable_t l1 = (pagetable_t)PTE2PA(*l2_pte);
      pte_t *l1_pte = &l1[PX(1, i)];
      
      // 如果是超级页（Level 1 的叶节点）
      if((*l1_pte & PTE_V) && PTE_LEAF(*l1_pte)) {
        szinc = SUPERPGSIZE;  // 2MB
...
#ifdef LAB_PGTBL
void
vmprint_get(pagetable_t pagetable, int level){
  for(int i = 0; i < 512; i++){
    pte_t pte = pagetable[i];
    ...
    if(pte & PTE_V){
      ...
      if((pte & (PTE_R|PTE_W|PTE_X)) == 0 && level < 2) {
        vmprint_get((pagetable_t)pa, level + 1);
      }
    }
  }
}
