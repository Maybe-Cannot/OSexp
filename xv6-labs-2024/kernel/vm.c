...
{
  // 写时复制实现: 不分配新物理页，父子共享，必要时置为只读+PTE_COW。
  for(uint64 i = 0; i < sz; i += PGSIZE){
    pte_t *pte = walk(old, i, 0);
    if(pte == 0)
      panic("uvmcopy: pte missing");
    if((*pte & PTE_V) == 0)
    ...
    // 若可写，改成只读并加COW标记。
    if(flags & PTE_W){
      *pte = (*pte & ~PTE_W) | PTE_COW; // 父pte去掉写，加cow
